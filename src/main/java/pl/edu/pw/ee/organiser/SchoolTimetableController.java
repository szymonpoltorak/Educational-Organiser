...
public class SchoolTimetableController extends MenuBarController {
    @FXML
    private AnchorPane timetableBox;
    @FXML
    ...
    public void initialize() throws IOException {
        ...
        var lessonArea = new ArrayList<TextArea>(50);
        var lessons = new File("DB/schoolTimetableDB/lessons");
        var lessonsHolder = new ArrayList<String>(50);
        var teachersHolder = new ArrayList<String>(50);
        var roomsHolder = new ArrayList<String>(50);

        int numBlock=0;
        try (var br = new BufferedReader(new FileReader(lessons))) {
            for (int i = 1; i < 6; i++) {
                for (int j = 1; j < 11; j++) {
                    ...
                    lessonArea.get(numBlock).focusedProperty().addListener((arg0, oldPropertyValue, newPropertyValue) -> {
                        if (!newPropertyValue) {
                            ...
                            try {
                                ...
                                System.err.println("Something wrong with write lessons to file database");
                            }
                        }
                    });
    ...
    public boolean setHoursAndDaysTable() {
        try {
            ...
            System.err.println("Something wrong with setHoursAndDaysTable");
            return false;
        }
    }
